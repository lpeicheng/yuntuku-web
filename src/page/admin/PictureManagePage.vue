<template>
  <div id="pictureManagePage">
    ...
    <a-form layout="inline" :model="searchParams" @finish="doSearch">
      ...
      <a-form-item>
        <a-button type="primary" html-type="submit" style="margin-right: 100px">搜索</a-button>
      </a-form-item>
      ...
      <a-flex justify="space-between">
        <a-space>
          ...
          <a-button type="primary"  @click="$router.push('/add_picture/batch')">+ 批量创建图片</a-button>
        </a-space>
      </a-flex>
    </a-form>
    <div style="margin-bottom: 16px;textAlign: center" />
    <!-- 表格 -->
    <a-table
      :columns="columns"
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'url'">
          <a-image :src="record.url" :width="{ maxWidth: '100', width: 'auto' }" />
        </template>
        ...
        <template v-else-if="column.key === 'action'">
          <a-space wrap>
            <a-button
              ...
              type="link"
              @click="handleReviewPass(record, PIC_REVIEW_STATUS_ENUM.PASS)"
            >
              通过
            </a-button>
            <a-button
...
const handleReviewPass = async (record: API.Picture, reviewStatus: number) => {
  const reviewMessage = reviewStatus === PIC_REVIEW_STATUS_ENUM.PASS ? '管理员操作通过' : '管理员操作拒绝'
  Modal.confirm({
    style: { width: '500px', height: '150px', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' },
    title: '确认通过',
